...
class DevicePlacement(object):
    ...
    def __init__(self, op_num, op_type_num, device_num, device_type_num, type_emb_size, device_emb_size,
                 max_output_shape_len, sample_size, dropout=0.5, learning_rate=0.001, reward_decay=0.9):
        self.op_num = op_num
    ...
    def define_device_placement_network(self):
        with tf.variable_scope("device_placement", reuse=tf.AUTO_REUSE):
            ...
            self.output_shape = tf.placeholder(dtype=tf.float32,
                                               shape=[batchsize, self.op_num, self.max_output_shape_len])
            ...
            def lstm_attention_decoder(embedding_decoder, hidden_size, dropout, num_layers, num_heads,
                                       attention_mechanism,
                ...
                decoder_initial_state = decoder_cell.zero_state(dtype=tf.float32, batch_size=batchsize).clone(
            ...
            decoder_input_embedding = tf.get_variable("decoder_input_embedding",
                                                      [self.device_type_num, self.device_emb_size])
            ...
            # actions
            self.decoder_target_data = tf.placeholder(tf.float32,
                                                      [self.sample_size, batchsize, self.op_num, self.device_num])
            encoder_outputs, final_encoder_state = lstm_bid_encoder(self.observations, HIDDEN_SIZE, num_layers,
                                                                    self.dropout, "encoder")
            # print(encoder_outputs, final_encoder_state)
            decoder_outputs, _ = lstm_attention_decoder(decoder_input_embedding, 2 * HIDDEN_SIZE, self.dropout,
                                                        num_layers, 1, attention_mechanism,
                                                        2 * HIDDEN_SIZE, True, "decoder", final_encoder_state,
                                                        encoder_outputs, self.start_token, self.output_layer,
                                                        self.op_num)
            ...
            self.neg_log_prob = tf.nn.softmax_cross_entropy_with_logits(logits=decoder_outputs,
                                                                        labels=self.decoder_target_data[0])
    ...
    def choose_action(self, type_emb, output_shape, adj, tgt_sos_id, sample_size, device_type_num, random_rate=0):
        ...
        prob_weights = self.sess.run(self.all_act_prob,
                                     feed_dict={self.input_data: type_emb, self.output_shape: output_shape,
                                                self.adj: adj, self.start_token: tgt_sos_id})
    ...
    def learn(self, type_emb, output_shape, adj, tgt_sos_id, actions, rewards):
        ...
        # train on episode
        _, all_act_prob, neg_log_prob, reward, apply_reward, loss = self.sess.run(
            [self.train_op, self.all_act_prob, self.neg_log_prob, self.reward, self.apply_reward, self.loss],
            feed_dict={
...
try:
    RL = DevicePlacement(op_num, op_type_num, device_num, device_type_num, type_emb_size, device_emb_size,
                         max_output_shape_len, sample_size, dropout=dropout_rate, learning_rate=lr)
except:
                ...
                if data is not None:
                    ...
                    for i in range(request_num):
                        if x is not None and y is not None and len(x) != 0 and len(y) != 0:
                            ...
                            all_act_prob, cross_entropy, reward, apply_reward, loss = RL.learn(type_emb,
                                                                                               output_shapes_arr, adjs,
                                                                                               tgt_sos_id, actions,
                                                                                               rewards)
                        ...
                        actions = RL.choose_action(type_emb, output_shapes_arr, adjs, tgt_sos_id, sample_size,
                                                   device_type_num, random_rate=random_rate)
